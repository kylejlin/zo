...
fn pop_and_reduce(states: &mut Vec<State>, nodes: &mut Vec<Node>, rule_kind: RuleKind) -> (Node, NonterminalKind) {
    match rule_kind {
        ...
        RuleKind::R14 => {
            ...
            let ind_2 = Box::new(Expr::try_from(nodes.pop().unwrap()).ok().unwrap());
            nodes.pop().unwrap();
            let lparen_0 = nodes.pop().unwrap().try_into_l_paren_0().ok().unwrap();
        ...
        RuleKind::R15 => {
            ...
            let cases_5 = Box::new(ZeroOrMoreMatchCases::try_from(nodes.pop().unwrap()).ok().unwrap());
...
const ACTION_TABLE: [[Action; 12]; 66] = [
    [
...
const GOTO_TABLE: [[Option<State>; 13]; 66] = [
    [
        Some(State::S35),
        Some(State::S10),
        None,
...
impl Node {
    ...
    fn try_into_ind_kw_2(self) -> Result<crate::token::ByteIndex, Self> {
        match self {
            Self::IndKw(t) => Ok(t),
            _ => Err(self),
        }
    ...
    fn try_into_vcon_kw_3(self) -> Result<crate::token::ByteIndex, Self> {
        match self {
            Self::VconKw(t) => Ok(t),
            _ => Err(self),
        }
    ...
    fn try_into_match_kw_4(self) -> Result<crate::token::ByteIndex, Self> {
        match self {
            Self::MatchKw(t) => Ok(t),
            _ => Err(self),
        }
    ...
    fn try_into_fun_kw_5(self) -> Result<crate::token::ByteIndex, Self> {
        match self {
            Self::FunKw(t) => Ok(t),
            _ => Err(self),
        }
    ...
    fn try_into_nonrec_kw_7(self) -> Result<crate::token::ByteIndex, Self> {
        match self {
    ...
    fn try_into_number_8(self) -> Result<crate::token::NumberLiteral, Self> {
        match self {
            Self::Number(t) => Ok(t),
            _ => Err(self),
        }
    ...
    fn try_into_string_9(self) -> Result<crate::token::StringLiteral, Self> {
        match self {
            Self::String(t) => Ok(t),
            _ => Err(self),
        }
    ...
    fn try_into_universe_10(self) -> Result<crate::token::UniverseLiteral, Self> {
        match self {
