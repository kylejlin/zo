...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum OptSquareBracketedExprs {
    None,
    Some(
...
enum NonterminalKind {
    ...
    Ind = 2,
    IndCommonInnards = 3,
    OptParenthesizedParamDefs = 4,
    ParenthesizedCommaSeparatedParamDefs = 5,
    CommaSeparatedParamDefs = 6,
    ParamDef = 7,
    OptDash = 8,
    OptSquareBracketedParamDefs = 9,
    SquareBracketedParamDefs = 10,
    ZeroOrMoreIndCases = 11,
    IndCase = 12,
    OptSquareBracketedExprs = 13,
    Fun = 14,
    FunCommonInnards = 15,
    Aind = 16,
    Vcon = 17,
    ...
    MatchCase = 25,
    OptParenthesizedCommaSeparatedIdentsOrUnderscores = 26,
    ParenthesizedCommaSeparatedIdentsOrUnderscores = 27,
    Afun = 28,
    OptIdent = 29,
    App = 30,
    VarOrApp = 31,
    CommaSeparatedExprs = 32,
    For = 33,
    IdentOrUnderscore = 34,
}
...
enum Node {
    ...
    IndCase(IndCase),
    OptSquareBracketedExprs(OptSquareBracketedExprs),
    Fun(Fun),
...
fn pop_and_reduce(states: &mut Vec<State>, nodes: &mut Vec<Node>, rule_kind: RuleKind) -> (Node, NonterminalKind) {
    match rule_kind {
        ...
        RuleKind::R11 => {
            ...
            let innards_2 = Box::new(IndCommonInnards::try_from(nodes.pop().unwrap()).ok().unwrap());
            let params_1 = Box::new(OptParenthesizedParamDefs::try_from(nodes.pop().unwrap()).ok().unwrap());
        ...
        RuleKind::R12 => {
            let universe_4 = nodes.pop().unwrap().try_into_universe_23().ok().unwrap();
            let returnkw_3 = nodes.pop().unwrap().try_into_return_kw_18().ok().unwrap();
            let cases_2 = Box::new(ZeroOrMoreIndCases::try_from(nodes.pop().unwrap()).ok().unwrap());
            let indices_1 = Box::new(OptSquareBracketedParamDefs::try_from(nodes.pop().unwrap()).ok().unwrap());
        ...
        RuleKind::R26 => {
            let return_type_3 = Box::new(OptSquareBracketedExprs::try_from(nodes.pop().unwrap()).ok().unwrap());
        ...
        RuleKind::R27 => {
            (
                Node::OptSquareBracketedExprs(OptSquareBracketedExprs::None),
                NonterminalKind::OptSquareBracketedExprs,
            )
        }
        RuleKind::R28 => {
            nodes.pop().unwrap();
            let t1 = Box::new(CommaSeparatedExprs::try_from(nodes.pop().unwrap()).ok().unwrap());
            nodes.pop().unwrap();
            
            states.truncate(states.len() - 3);
            
            (
                Node::OptSquareBracketedExprs(OptSquareBracketedExprs::Some(
                    t1,
                )),
                NonterminalKind::OptSquareBracketedExprs,
            )
        ...
        RuleKind::R38 => {
            let t0 = Box::new(SquareBracketedCommaSeparatedIdentsOrUnderscores::try_from(nodes.pop().unwrap()).ok().unwrap());
        ...
        RuleKind::R39 => {
            let t2 = Box::new(SquareBracketedCommaSeparatedIdentsOrUnderscores::try_from(nodes.pop().unwrap()).ok().unwrap());
            nodes.pop().unwrap();
            let t0 = nodes.pop().unwrap().try_into_ident_20().ok().unwrap();
...
const ACTION_TABLE: [[Action; 26]; 109] = [
    [
        Action::Err,
        Action::Err,
        Action::Err,
...
const GOTO_TABLE: [[Option<State>; 35]; 109] = [
    [
        Some(State::S27),
        Some(State::S8),
        Some(State::S16),
...
impl TryFrom<Node> for OptSquareBracketedExprs {
    type Error = Node;

    fn try_from(node: Node) -> Result<Self, Self::Error> {
        match node {
            Node::OptSquareBracketedExprs(n) => Ok(n),
            _ => Err(node),
        }
    }
}
