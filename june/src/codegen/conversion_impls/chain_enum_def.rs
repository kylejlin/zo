...
impl JuneConverter {
    pub(crate) fn convert_chain_enum_def(
        &mut self,
        expr: &jnode::ChainEnumDef,
        context: Context,
    ) -> Result<znode::Expr, SemanticError> {
        let ind = self.convert_standalone_enum_def(&expr.standalone, context)?;

        let vcon_extension = self.get_enum_case_definitions(&expr.standalone, context)?;
    ...
    fn get_enum_case_definitions<'a>(
        &mut self,
        expr: &'a jnode::EnumDef,
        context: Context,
...
impl JuneConverter {
    pub(crate) fn convert_standalone_enum_def(
        &mut self,
        expr: &jnode::EnumDef,
        context: Context,
    ) -> Result<znode::Expr, SemanticError> {
        match &*expr.params {
            jnode::OptParenthesizedNonfunParamDefs::None => {
                self.convert_unparameterized_enum_def(&expr, context)
            }

            jnode::OptParenthesizedNonfunParamDefs::Some(parenthesized) => {
                self.convert_parameterized_enum_def(&expr, &parenthesized.params, context)
            }
        }
    }

    fn convert_unparameterized_enum_def(
        &mut self,
        expr: &jnode::EnumDef,
        context: Context,
    ) -> Result<znode::Expr, SemanticError> {
        let ind = self.convert_unparameterized_enum_def_to_zo_ind(expr, context)?;
        Ok(self.cache_ind(ind))
    }

    fn convert_parameterized_enum_def(
        &mut self,
        expr: &jnode::EnumDef,
        params: &jnode::CommaSeparatedNonfunParamDefs,
        context: Context,
    ) -> Result<znode::Expr, SemanticError> {
        ...
        let return_type_ind =
            self.convert_unparameterized_enum_def_to_zo_ind(expr, context_with_params)?;

        let enum_type_cfor = znode::For {
        ...
        let return_val = self.convert_unparameterized_enum_def_to_zo_ind(
            expr,
        ...
        Ok(self.cache_fun(znode::Fun {
            ...
            param_types,
            return_type: enum_type_cfor,
            return_val,
            aux_data: (),
        }))
    }

    pub(crate) fn convert_unparameterized_enum_def_to_zo_ind(
        &mut self,
        expr: &jnode::EnumDef,
        context: Context,
        ...
        let name = self.cache_string_value(StringValue(expr.name.value.clone()));
        ...
        let vcon_defs = self.convert_ordered_enum_cases(&cases, context_with_recursive_ind)?;
    ...
    fn convert_ordered_enum_cases(
        &mut self,
        cases: &[&jnode::EnumCase],
        context: Context,
    ) -> Result<RcHashedVec<znode::VconDef>, SemanticError> {
        let v: Vec<znode::VconDef> = cases
            .into_iter()
            .map(|case| self.convert_enum_case(case, context))
            .collect::<Result<Vec<_>, _>>()?;
        Ok(bypass_cache_and_rc_hash(v))
    }

    fn convert_enum_case(
        &mut self,
        case: &jnode::EnumCase,
        context: Context,
...
impl JuneConverter {
    pub(crate) fn convert_vcon_with_valid_vcon_index(
        ...
        match &*expr.params {
            jnode::OptParenthesizedNonfunParamDefs::None => self
            ...
            jnode::OptParenthesizedNonfunParamDefs::Some(parenthesized) => self
    ...
    fn convert_parameterized_vcon_with_valid_vcon_index(
        ...
        let context_with_params_and_recursive_fun =
            Context::Snoc(&context_with_params, &unusable_singleton);
        let return_val_ind = self.convert_unparameterized_enum_def_to_zo_ind(
            &expr,
            context_with_params_and_recursive_fun,
        )?;
